...
import { Search, Clock, Check, X, Settings, FilterHorizontal, Package, User, Phone, Calendar, ChevronRight, Factory } from '@/utils/icons';
import { colors } from '@/constants/colors';
import { useApp } from '@/contexts/AppContext';
import { ProductionStatus, Order } from '@/types';
import { ProductionSkeletonLoader } from '@/components/Skeleton';
import { formatFullCurrency, formatDate, getStatusLabel } from '@/utils/format';
...
const filters: { key: FilterType; label: string; icon: React.FC<{ size?: number; color?: string }> }[] = [
  { key: 'semua', label: 'Semua', icon: Factory },
  { key: 'antrian', label: 'Antrian', icon: Clock },
  { key: 'diproses', label: 'Diproses', icon: Package },
  { key: 'siap_diambil', label: 'Siap', icon: Check },
];
...
const statusFilterOptions: { key: ProductionStatus | 'semua'; label: string }[] = [
...
function getProductionStatusColor(status: ProductionStatus): string {
  switch (status) {
    case 'antrian': return colors.warning;
    case 'diproses': return colors.primary;
    case 'siap_diambil': return colors.success;
    case 'selesai': return colors.info;
    default: return colors.textTertiary;
  }
}

function getProductionStatusLabel(status: ProductionStatus): string {
  switch (status) {
...
function ProductionItem({ order, onPress, isLast = false }: ProductionItemProps) {
  const statusColor = order.status === 'terlambat' ? colors.error : getProductionStatusColor(order.productionStatus);

  return (
    <TouchableOpacity style={styles.row} onPress={onPress} activeOpacity={0.6}>
      <View style={[styles.itemContent, !isLast && styles.itemContentBorder]}>
...
export default function ProductionScreen() {
  ...
  const [filterStatus, setFilterStatus] = useState<ProductionStatus | 'semua'>('semua');
  ...
  const [tempStatus, setTempStatus] = useState<ProductionStatus | 'semua'>('semua');
  ...
  const productionOrders = useMemo(() => orders.filter(o => o.productionStatus !== 'selesai'), [orders]);
  const availableYears = useMemo(() => getAvailableYears(productionOrders), [productionOrders]);
  ...
  const getFilterCount = (filter: FilterType) => {
    if (filter === 'semua') return productionOrders.length;
  ...
  const getOrdersForFilter = (filter: FilterType): Order[] => {
    let filtered = filter === 'semua'
      ? productionOrders
      : orders.filter(o => o.productionStatus === filter);
    ...
    if (filterStatus !== 'semua') {
      filtered = filtered.filter(order => order.productionStatus === filterStatus);
    }
    if (filterYear !== 'Semua') {
    ...
    filteredOrders.forEach(order => {
      ...
      switch (filterGroup) {
        ...
        case 'telepon':
          ...
          break;
        case 'status':
          key = getProductionStatusLabel(order.productionStatus);
          break;
        case 'layanan':
              ...
              >
                {filterOrders.length === 0 ? (
                  <View style={styles.emptyState}>
                    <Factory size={48} color={colors.border} />
                    <Text style={styles.emptyTitle}>Tidak ada pesanan</Text>
                    <Text style={styles.emptyText}>Belum ada pesanan produksi untuk kategori ini</Text>
                  </View>
                ) : (
      ...
      >
        ...
        >
          ...
          <View style={styles.sheetHeader}>
            <Text style={styles.sheetTitle}>Filter Produksi</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={styles.sheetScroll}>
            ...
            <Text style={styles.sheetSectionTitle}>Status Produksi</Text>
            <View style={styles.chipRow}>
